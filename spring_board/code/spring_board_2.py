...
import pandas as pd
urlArray = []
urlArray.append("https://www.springboard.com/courses/ai-machine-learning-career-track/")
urlArray.append("https://www.springboard.com/courses/data-science-career-track/")
urlArray.append("https://www.springboard.com/courses/data-analytics-career-track/")
urlArray.append("https://www.springboard.com/courses/ux-career-track/")

def process_description(soupObject):
    desc = ""
    descrip = soupObject.find('div', attrs={'class':'styles__SubHeader-sc-13r03wa-2 gsGfzS'})
    for des in descrip:
...
def process_prereq(soupObject):
    pre = ""
    prereq = soupObject.find('div', attrs={'class':'prereq'}).find_all('span')
    for pree in prereq:
        pre = pre + pree.getText() + "|"
    return pre
...
def process_reviewer_2_name(soupObject):
    name = ""
    name_rev = soupObject.find('div', attrs={'class':'name'}).findNext('div', attrs={'class':'name'}).getText()
    for namm in name_rev:
        name = name + namm
    return name
...
def process_review_2_review(soupObject):
    review =""
    revv = soupObject.find('div', attrs={'class':'avatar-quote'}).findNext('div', attrs={'class':'avatar-quote'}).getText()
    for reev in revv:
        review = review + reev
    return review
...
def process_reviewer_2_image(soupObject):
    rev_image_Data = ""
    rev_image_Data_tag = soupObject.find('div', attrs={'class':'avatar-image'}).findNext('div', attrs={'class':'avatar-image'})
    if (rev_image_Data_tag is None):
        return rev_image_Data
    for img in rev_image_Data_tag.findAll('img'):
        rev_image_Data = rev_image_Data + img['src']
...
def process_course_duration(soupObject):
    duration=""
    dur = soupObject.find('span',attrs={'class':'description'}).findNext('strong')
    for durr in dur:
...
def process_regular(soupObject):
    reg_price = ""
    regg = soupObject.find('div',attrs={'class':'styles__PriceWrapper-tkd64p-2 kknsVJ'}).findChildren('div',attrs={'class':'subtext'})
    for pricee in regg:
        reg_price = reg_price + pricee.getText()
    return reg_price

def process_sale_price(soupObject):
    sale_price =""
    salee = soupObject.find('div',attrs={'class':'styles__PriceWrapper-tkd64p-2 kknsVJ'}).findChildren('div',attrs={'class':'actualPrice'})
    for sale_pc in salee:
        sale_price = sale_price + sale_pc.getText()
    return sale_price
...
df.to_csv('spring_board_2.csv', index=True, header=True)